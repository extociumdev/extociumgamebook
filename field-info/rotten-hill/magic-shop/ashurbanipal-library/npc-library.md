---
description: 🛑 Information on this page may differ from the current in-game data.
---

# 😎 NPC - Library

<figure><img src="../../../../.gitbook/assets/KakaoTalk_20230824_120313487_01.png" alt="" width="205"><figcaption></figcaption></figure>

## �&#xDFE2;_&#x54;he Gatekeeper of Succession_

{% tabs %}
{% tab title="ENG" %}
#### **SEHEIAH**

> _The Gatekeeper of Succession_&#x20;

Seheiah can be found in the **Ashurbanipal Library**,\
located on the second floor of the Magic Shop in Rotten Hill.

She oversees the ritual of **Hero Succession**,\
guiding heroes to ascend to a higher stage of existence.

If you are ready to take your hero beyond their current limits,\
Seheiah will open the path forward.

👇 Learn more on the **Hero Succession page**.

{% content-ref url="../../../../growth/hero-ascension/succession/" %}
[succession](../../../../growth/hero-ascension/succession/)
{% endcontent-ref %}
{% endtab %}

{% tab title="한국어" %}
#### 세헤이아

> _영혼의 문지기_

세헤이아는 로튼힐의 **마법상점 2층**, **아슈르바니팔 도서관**에서 만날 수 있는 NPC입니다.\
세헤이아는 **영웅 계승**을 관장하며, 영웅을 더 높은 단계로 인도하는 역할을 맡고 있습니다.\
현재의 한계를 넘어 다음 단계로 나아갈 준비가 되었다면, 세헤이아를 찾아가 보세요.

👇 자세한 내용은 **영웅 계승 가이드 페이지**에서 확인할 수 있습니다.

{% content-ref url="../../../../growth/hero-ascension/succession/" %}
[succession](../../../../growth/hero-ascension/succession/)
{% endcontent-ref %}
{% endtab %}

{% tab title="日本語" %}
#### セヘイア

> _魂の門番_

セヘイアは、ロッテンヒルの **マジックショップ2階にあるアシュルバニパル図書館**で\
出会うことができるNPCです。\
彼女は **英雄継承** を司り、英雄をより高い段階へと導く役割を担っています。\
現在の限界を超え、次の段階へ進む準備ができたなら、セヘイアを訪れてみましょう。

👇 詳細は **英雄継承ガイドページ**をご確認ください。

{% content-ref url="../../../../growth/hero-ascension/succession/" %}
[succession](../../../../growth/hero-ascension/succession/)
{% endcontent-ref %}
{% endtab %}
{% endtabs %}



***

<figure><img src="../../../../.gitbook/assets/KakaoTalk_20230824_120313487_06.png" alt="" width="205"><figcaption></figcaption></figure>

## �&#xDFE2;_&#x54;he Weaver of Synthesis_

{% tabs %}
{% tab title="ENG" %}
#### **CHAVAKIAH**

> _The Weaver of Synthesis_&#x20;

**Chavakiah** can be found in the **Ashurbanipal Library**,\
located on the second floor of the Magic Shop in Rotten Hill.\
He specializes in **Hero Synthesis**, combining multiple heroes into a single, more powerful one.

If you want to take your heroes to the next level, Chavakiah is the one to visit.

👇 Learn more on the **Hero Synthesis page**.

{% content-ref url="../../../../growth/hero-ascension/synthesis.md" %}
[synthesis.md](../../../../growth/hero-ascension/synthesis.md)
{% endcontent-ref %}
{% endtab %}

{% tab title="한국어" %}
#### 차바키아

> _합성의 설계자_

차바키아는 로튼힐의 **마법상점 2층**, **아슈르바니팔 도서관**에서 만날 수 있는 NPC입니다.\
차바키아는 **영웅 합성**을 전문으로 하며, 영웅을 하나로 합쳐 더 강력한 영웅을 만들어 냅니다.

보유한 영웅을 한 단계 더 강한 존재로 만들고 싶다면, 차바키아를 찾아가 보세요.

👇 Learn more on the **Hero Synthesis page**.

{% content-ref url="../../../../growth/hero-ascension/synthesis.md" %}
[synthesis.md](../../../../growth/hero-ascension/synthesis.md)
{% endcontent-ref %}
{% endtab %}

{% tab title="日本語" %}
#### チャヴァキア

> _合成の設計者_&#x20;

**チャヴァキア**は、ロッテンヒルのマジックショップ2階にある\
**アシュルバニパル図書館**で出会えるNPCです。

チャヴァキアは **英雄合成** を専門としており、\
英雄を一つに統合して、より強力な英雄を生み出します。

所持している英雄を さらに強い存在へと成長させたい場合は、\
チャヴァキアを訪れてみてください。

👇 詳細は **英雄合成ガイドページ**をご確認ください。

{% content-ref url="../../../../growth/hero-ascension/synthesis.md" %}
[synthesis.md](../../../../growth/hero-ascension/synthesis.md)
{% endcontent-ref %}
{% endtab %}
{% endtabs %}



***

<figure><img src="../../../../.gitbook/assets/KakaoTalk_20230824_120313487_10.png" alt="" width="205"><figcaption></figcaption></figure>

## �&#xDFE2;_&#x54;he Keeper of Rebirth_

{% tabs %}
{% tab title="ENG" %}
#### LELAHEL

> _The Keeper of Rebirth_

**Lelahel** can be found in the **Ashurbanipal Library**,\
located on the second floor of the Magic Shop in Rotten Hill.

As the **Keeper of Rebirth**, \
Lelahel allows you to **reload a Hero back to its original state** in exchange for a cost.\
Please note that **all progress, including levels and growth, will be fully reset**.

If you wish to start over and explore a new path for your Hero,\
Lelahel is the one to seek.

👇 _Go to the Hero Reload page for details._

{% content-ref url="../../../../growth/hero-ascension/reload/" %}
[reload](../../../../growth/hero-ascension/reload/)
{% endcontent-ref %}
{% endtab %}

{% tab title="한국어" %}
#### 레라엘

> _재탄생의 수호자_

레라엘은 로튼힐의 마법상점 2층에 위치한 아슈르바니팔 도서관에서 만날 수 있는 NPC입니다.

재탄생의 수호자인 레라엘은, 대가를 지불하면 영웅을 태초의 상태로 리로드할 수 있도록 돕습니다.\
리로드 시 레벨과 성장도를 포함한 모든 진행 상황이 완전히 초기화되니 주의해 주세요.

영웅의 여정을 처음부터 다시 시작하고 새로운 성장 경로를 탐색하고 싶다면,\
레라엘을 찾아가 보세요.

👇 _자세한 내용은 영웅 리로드 페이지에서 확인할 수 있습니다._

{% content-ref url="../../../../growth/hero-ascension/reload/" %}
[reload](../../../../growth/hero-ascension/reload/)
{% endcontent-ref %}
{% endtab %}

{% tab title="日本語" %}
#### レラエル

> _再生の守護者_

レラエルは、ロッテンヒルのマジックショップ2階にある\
アシュルバニパル図書館で出会うことができます。

再生の守護者であるレラエルは、\
代価を支払うことでヒーローを初期状態へリロードすることができます。

リロードを行うと、レベルや成長状況を含む\
すべての進行内容が完全にリセットされますのでご注意ください。

ヒーローの旅を最初からやり直し、新たな成長の道を探したい場合は、\
レラエルを訪ねてみてください。

👇 _詳細はヒーローリロードページをご確認ください。_

{% content-ref url="../../../../growth/hero-ascension/reload/" %}
[reload](../../../../growth/hero-ascension/reload/)
{% endcontent-ref %}
{% endtab %}
{% endtabs %}



***

<figure><img src="../../../../.gitbook/assets/NPC_Omael.png" alt="" width="205"><figcaption></figcaption></figure>

## �&#xDFE2;_&#x54;he Rune Craftsman_

{% tabs %}
{% tab title="ENG" %}
#### **OMAEL**

> _The Rune Craftsman_

Omael can be found in the Ashurbanipal Library,\
located on the second floor of the Magic Shop in Rotten Hill.

Once you acquire a [**Succession Hero**](../../../../growth/hero-ascension/succession/), you gain access to Skill Runes.\
Omael specializes in crafting Skill Runes using the materials you have collected during your adventures.

If you want to unlock new skill potential and enhance your Hero’s combat style,\
Omael is the one to seek.

👇 Go to the Skill Rune page for details.

{% content-ref url="../../../../growth/hero-ascension/succession/skill-rune-crafting.md" %}
[skill-rune-crafting.md](../../../../growth/hero-ascension/succession/skill-rune-crafting.md)
{% endcontent-ref %}
{% endtab %}

{% tab title="한국어" %}
#### 오마엘

> _룬 제작자_

오마엘은 로튼힐의 마법상점 2층에 위치한 아슈르바니팔 도서관에서 만날 수 있는 NPC입니다.

[**계승 영웅**](../../../../growth/hero-ascension/succession/)을 획득하면 스킬 룬 시스템을 사용할 수 있으며,\
오마엘은 모험을 통해 수집한 재료를 사용해 스킬 룬을 제작해 주는 전문가입니다.

영웅의 스킬을 더욱 강화하고 전투 스타일을 확장하고 싶다면, 오마엘을 찾아가 보세요.

👇 자세한 내용은 스킬 룬 안내 페이지에서 확인할 수 있습니다.

{% content-ref url="../../../../growth/hero-ascension/succession/skill-rune-crafting.md" %}
[skill-rune-crafting.md](../../../../growth/hero-ascension/succession/skill-rune-crafting.md)
{% endcontent-ref %}
{% endtab %}

{% tab title="日本語" %}
#### オマエル

> _ルーン職人_

オマエルは、ロッテンヒルのマジックショップ2階にある\
アシュルバニパル図書館で出会うことができます。

[**継承ヒーロー**](../../../../growth/hero-ascension/succession/)を獲得すると、スキルルーンシステムが解放されます。\
オマエルは、冒険で集めた素材を使って スキルルーンを作成する専門家です。

ヒーローのスキルを強化し、戦闘スタイルの幅を広げたい場合は、\
オマエルを訪ねてみてください。

👇 詳細はスキルルーン案内ページをご確認ください。

{% content-ref url="../../../../growth/hero-ascension/succession/skill-rune-crafting.md" %}
[skill-rune-crafting.md](../../../../growth/hero-ascension/succession/skill-rune-crafting.md)
{% endcontent-ref %}
{% endtab %}
{% endtabs %}

<p align="right"><sup><em>※ This guide was written based on the game status as of January 21, 2026,</em></sup> <br><sup><em>and its contents may change with future updates.</em></sup></p>
